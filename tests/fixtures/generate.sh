...
"$GO_DOTENV" -f godotenv.env node dumpenv.js > edge_cases_godotenv.rs

#dotenvy --file=dotenvy.env node dumpenv.js > dotenvy.rs

pushd java
gradle -q run --args="--file ../java.env node ../dumpenv.js" > ../edge_cases_java.rs
popd

pushd ../..
cargo run -- --strict=false --file=tests/fixtures/edge-cases.env node tests/fixtures/dumpenv.js | sed 's/:\\b/:\\u{8}/g' | sed 's/:\\f/:\\u{C}/g' > tests/fixtures/edge_cases_punktum.rs
popd
